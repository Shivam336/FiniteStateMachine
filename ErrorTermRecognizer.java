...
public class ErrorTermRecognizer {
	...
	private static void displayDebuggingInfo() {
		if (currentCharNdx >= inputLine.length())
			System.out.println(((state < 11) ? "  " : " ") + state + 
					((finalState) ? "       F   " : "           ") + "None");
		else
			System.out.println(((state < 11) ? "  " : " ") + state + 
					((finalState) ? "       F   " : "           ") + "  " + currentChar + " " + 
					((nextState < 11) && (nextState != -1) ? "    " : "   ") + nextState );
	}
	...
	public static String checkErrorTerm(String input) {
		...
		while (running) {
			...
			case 8: 
				// State 8 has one valid transitions.
				
				// This is a not final state
				finalState = false;
				
				// If the current character is ., it transitions to state 9	
				if (currentChar == '.'){
					nextState = 9;
					break;
				}
		...
		case 3:
			// States  3 is not a final state and in order to
			// move forward, the next character must be a digit.
			errorTermErrorMessage = "This character may only be a digit from 1 to 9.\n";
			return errorTermErrorMessage + displayInput(input, currentCharNdx);
			
		case 6:
			// States  6 is not a final state and in order to
			// move forward, the next character must be a digit.
		...
		case 8:
			...
			errorTermIndexofError = currentCharNdx;		// Copy the index of the current character;
			errorTermErrorMessage = "This character must be a decimal point.";
			return errorTermErrorMessage + displayInput(input, currentCharNdx);
	
		case 9:
			// State 9 is not a final state, so we can return a very specific error message
			errorTermIndexofError = currentCharNdx;		// Copy the index of the current character;
			errorTermErrorMessage = "This character must be a digit from 1 to 9";
			return errorTermErrorMessage + displayInput(input, currentCharNdx);	
	}
		return input;
